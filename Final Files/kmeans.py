...
def pre_process(df):
	...
	kmeans = MiniBatchKMeans(n_clusters=2,init = 'k-means++',random_state=0, batch_size=1000,verbose = 0,max_iter = 1000)
	kmeans = kmeans.partial_fit(X_train_tfidf)
	#kmeans = kmeans.partial_fit(train_data_x)
	
	pipeline = Pipeline(([('vect', CountVectorizer()),('tfidf', TfidfTransformer()), ('kmeans', MiniBatchKMeans())]))
	pipeline.fit(feature1)
...
def plot_result(x_name, label_name, df):
	# sns.scatterplot(data=df, x=range(len(df)), y=x_name, hue=label_name)
	sns.stripplot(x=label_name, y=x_name, data=df)
	plt.show()
